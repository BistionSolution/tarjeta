...
<?php 
    ...
    foreach($name_last as $nl):
        $hiden_ref = $nl->username;
        echo "<h2>Total de Cuentas de: ".ucwords($nl->first_name." ".$nl->last_name." username:".$nl->username)."</h2>";   
    endforeach;
    ...
    foreach ($product_order as $product):
        ...
        if (empty($vcar)){ 
            $i = 1;
            do{
                do_shortcode("[insert_contact order={$product->order_id} product={$product->product_id} customer={$id} hiden_ref={$hiden_ref}]");
                $i = $i + 1;
            }
